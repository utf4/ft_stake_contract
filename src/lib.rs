...
pub fn process_instruction(
    ...
    let admin = "AQfNDHq4kMeCVjivwkNGmqL7MxhrmaXsf6cYYmRQsKyc".parse::<Pubkey>().unwrap();
    ...
    match instruction{
        StakeInstruction::Unstake=>{
            ...
            if clock.unix_timestamp as u64-stake_data.timestamp < rate.min_lock_period{
                //can't unstake because minimal period of staking is not reached yet
                // return Err(ProgramError::Custom(0x109));
                // let deflation = stake_data.amount*90/100;
                ...
                // )?;
                if stake_data.amount >= 2000 && stake_data.amount <= 29999{
                    msg!("bronze package");    
                    msg!("stake_data.amount {:?}",stake_data.amount);
                    let deflation =(stake_data.amount*48)/10000;
                    let deduction = stake_data.amount-((stake_data.amount*48)/10000);                   
                    msg!("deflation {:?}",deflation);
                    if payer_mint_holder_info.owner != token_info.key{
                        invoke(
                            &spl_associated_token_account::create_associated_token_account(
                                payer.key,
            ...
            } else if stake_data.amount >= 30000 && stake_data.amount <= 224999 {
                    msg!("silver package");
                    msg!("stake_data.amount {:?}",stake_data.amount);
                    let deflation =(stake_data.amount*64)/10000;
                    let deduction = stake_data.amount-((stake_data.amount*64)/10000);                     
                    msg!("deflation {:?}",deflation);
            ...
            } else if stake_data.amount >= 225000 && stake_data.amount <= 299999 {
                    msg!("gold package");
                    msg!("stake_data.amount {:?}",stake_data.amount);
                    let deflation =(stake_data.amount*85)/10000;
                    let deduction = stake_data.amount-((stake_data.amount*85)/10000);    
                    msg!("deflation {:?}",deflation);                 
            ...
            } else if stake_data.amount >= 300000 && stake_data.amount <= 499999 {
                    msg!("diamond package");
                    msg!("stake_data.amount {:?}",stake_data.amount);
                    let deflation =(stake_data.amount*96)/10000;
                    let deduction = stake_data.amount-((stake_data.amount*96)/10000);                     
                    msg!("deflation {:?}",deflation);
            ...
            } else if stake_data.amount >= 500000 { 
                    msg!("challenger package");
                    msg!("stake_data.amount {:?}",stake_data.amount);
                    let deflation =(stake_data.amount*115)/10000;
                    let deduction = stake_data.amount-((stake_data.amount*115)/10000);                     
                    msg!("deflation {:?}",deflation);
            ...
            if stake_data.amount >= 2000 && stake_data.amount <= 29999{
                msg!("bronze package");    
                let reward_percent = 480;
                msg!("reward_percent {:?}",reward_percent);
                msg!("stake_data.amount {:?}",stake_data.amount);
                let temp_reward = ((stake_data.amount*reward_percent)/10000)*1000000;
                msg!("temp_reward {:?}",temp_reward);
                msg!("seconds passes {:?}",clock.unix_timestamp as u64-stake_data.timestamp);
                //365 seconds equal to one year --- so in lock period 1 is 1 day
                let period = (clock.unix_timestamp as u64-stake_data.timestamp)/rate.apy_inc_period;
                let total_reward = ((temp_reward/365)*period)/1000000;
                // 31556926 seconds equal to one year
                //let total_reward = ((temp_reward/ 31556926)*(stake_data.lock_period))/1000000000;
            ...
            invoke_signed(
                ...
                msg!("time period passed {:?}",(clock.unix_timestamp as u64-stake_data.timestamp)/rate.apy_inc_period);//seconds_passed/apy_increament _period
                let reward_percent = 640;
                msg!("reward_percent {:?}",reward_percent);
                msg!("stake_data.amount {:?}",stake_data.amount);
                let temp_reward = ((stake_data.amount*reward_percent)/10000)*1000000;
                msg!("temp_reward {:?}",temp_reward);
            ...
            invoke_signed(
                ...
                msg!("seconds passed {:?}",clock.unix_timestamp as u64-stake_data.timestamp);//seconds_passed
                msg!("time period passed {:?}",(clock.unix_timestamp as u64-stake_data.timestamp)/rate.apy_inc_period);//seconds_passed/apy_increament _period
                let reward_percent = 8530;
                msg!("reward_percent {:?}",reward_percent);
                msg!("stake_data.amount {:?}",stake_data.amount);
                let temp_reward = ((stake_data.amount*reward_percent)/100000)*1000000;
                msg!("temp_reward {:?}",temp_reward);
                msg!("seconds passes {:?}",clock.unix_timestamp as u64-stake_data.timestamp);
                //365 seconds equal to one year --- so in lock period 1 is 1 day
                let period = (clock.unix_timestamp as u64-stake_data.timestamp)/rate.apy_inc_period;
            ...
            panic!("you have staked an inapropriate amount");
        }
